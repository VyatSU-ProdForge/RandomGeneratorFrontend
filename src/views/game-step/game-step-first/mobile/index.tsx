...
import styles from './styles/game-step-first-mobile.module.scss';

export function GameStepFirstMobile(): React.ReactElement {
  ...
  // Загружаем SVG курсор как изображение
  React.useEffect(() => {
    const img = new Image();
    img.src = cursorSvg;
    img.onload = (): void => {
  ...
  // Инициализация canvas с сеткой и курсором в центре
  React.useEffect(() => {
    ...
    // Рисуем сетку из точек на drawing canvas
  ...
  // Перерисовываем курсор при изменении позиции
  React.useEffect(() => {
